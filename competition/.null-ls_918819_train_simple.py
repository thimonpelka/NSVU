...
def train_enhanced_model_manual_progress(X_train, y_train):
    """Train model with manual progress tracking"""
    print("Training enhanced model with manual progress tracking...")
    ...
    with tqdm(param_combinations, desc="Testing parameters", unit="combo") as pbar:
        for param_combo in pbar:
            ...
            pbar.set_postfix(
                {"Best Score": f"{max(best_score, mean_score):.4f}"})
...
if __name__ == "__main__":
    ...
    # # Entferne konstanten Features (VarianceThreshold)
    # constant_filter = VarianceThreshold(threshold=0.0)  # entfernt nur wirklich konstante Spalten
    # X = constant_filter.fit_transform(X)
    #
    # # Optional: Wenn du wissen willst, welche Features entfernt wurden:
    # selected_features = constant_filter.get_support(indices=True)
    # print(f"Verbleibende Features nach Konstanzfilter: {len(selected_features)}")
    ...
    # Train model
    best_model = train_enhanced_model_manual_progress(X_train, y_train)
